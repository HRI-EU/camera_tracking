...
class kinectBodyTracker:
    def __init__(self, modulePath, sensor_calibration, modelType, model_directory):
        ...
        if modelType == 1:
            model_path = os.path.join(model_directory, "dnn_model_2_0_lite_op11.onnx")
            if os.path.exists(model_path):
                _k4abt.K4ABT_TRACKER_CONFIG_DEFAULT.model_path = model_path.encode("utf-8")
            else:
                print("Failed to find lite model. Using the original body tracking model.")
    ...
    def draw2DSkeleton(self, skeleton2D, bodyId, image):
        ...
        for segmentId in range(len(_k4abt.K4ABT_SEGMENT_PAIRS)):
            ...
            image = cv2.line(image, (int(point1[0]), int(point1[1])), (int(point2[0]), int(point2[1])), (255, 0, 0), 2)
    ...
    def enqueue_capture(self, capture_handle, timeout_in_ms=_k4a.K4A_WAIT_INFINITE):
        ...
        """
        _k4abt.VERIFY(
            self.k4abt.k4abt_tracker_enqueue_capture(self.tracker_handle, capture_handle, timeout_in_ms),
    ...
    def get_body_skeleton(self, index=0):
        ...
        _k4abt.VERIFY(
            self.k4abt.k4abt_frame_get_body_skeleton(self.body_frame_handle, index, skeleton),
